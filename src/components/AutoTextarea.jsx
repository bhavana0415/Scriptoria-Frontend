...
const AutoTextarea = ({ item, ind, handleChange, changeType }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="absolute -top-3 right-2 w-full h-fit flex flex-row-reverse gap-1 sm:gap-2">
        {["Chapter", "Heading", "Subheading", "Paragraph"].map((type) => (
          <button key={type} onClick={() => changeType(ind, type)}>
            <p className="rounded-full w-6 h-6 border-2 border-pink-50 bg-pink-800 text-pink-50 cursor-pointer flex justify-center items-center">
              {type.charAt(0)}
            </p>
          </button>
        ))}
